...
void DefaultDebugger_cli(YDebug* debug, YThread* th) {
	th->frame->debug_flags &= ~(1 << NEXT_FLAG);
	th->frame->debug_flags &= ~(1 << STEP_FLAG);
	DefaultDebugger* dbg = (DefaultDebugger*) debug;
	bool work = true;
#define CMD(l, cmd) if (wcscmp(l, cmd)==0)
	while (work) {
		ILProcedure* proc =
				th->runtime->bytecode->procedures[th->frame->proc->id];
		CodeTableEntry* e = proc->getCodeTableEntry(proc, th->frame->pc);
		if (e != NULL) {
		...
		CMD(argv[0], L"run") {
			...
			th->frame->debug_flags |= 1 << NEXT_FLAG;
			if (e != NULL) {
				th->frame->debug_ptr = (void*) e;
			} else
				th->frame->debug_ptr = NULL;
			work = false;
			...
			if (argc > 1) {
				...
				CMD(req, L"files") {
					...
					int32_t pid = th->frame != NULL ? th->frame->proc->id : -1;
					if (argc == 3)
			...
			if (argc > 1) {
				...
				if (res.pid != -1) {
					...
					YValue* val = invoke(res.pid, (YObject*) th->frame->regs[0],
					NULL, th);
	...
	if (e != NULL) {
		for (size_t i = 0; i < dbg->bp_count; i++) {
			...
			if (dbbp->fileid == e->file && dbbp->line == e->line) {
				if ((th->frame->debug_flags & (1 << LINE_FLAG)) == 0
						|| th->frame->debug_field != dbbp->line) {
					...
					if (dbbp->condition != NULL) {
						...
						if (res.pid != -1) {
							...
							YValue* val = invoke(res.pid,
									(YObject*) th->frame->regs[0], NULL, th);
							th->exception = NULL;
				...
				th->frame->debug_flags |= 1 << LINE_FLAG;
				th->frame->debug_field = dbbp->line;
				return;
			}
		}
		if ((th->frame->debug_flags & (1 << STEP_FLAG)) == 1 << STEP_FLAG) {
			CodeTableEntry* last_e = (CodeTableEntry*) th->frame->debug_ptr;
			if (last_e != NULL && last_e->line != e->line) {
				th->frame->debug_flags &= ~(1 << STEP_FLAG);
			}
		}
		if ((th->frame->debug_flags & (1 << NEXT_FLAG)) == 1 << NEXT_FLAG) {
			CodeTableEntry* last_e = (CodeTableEntry*) th->frame->debug_ptr;
			if (last_e == NULL)
				DefaultDebugger_cli(debug, th);
			if (last_e != NULL && last_e->line != e->line) {
