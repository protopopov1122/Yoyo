...
static void save_err_str(const CHAR *str) {
	...
	pos += FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwMessageId,
			MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), error_buffer + pos,
			sizeof(error_buffer) - pos, NULL);
...
void *dlopen(const char *file, int mode) {
	...
	if (file == 0) {
		 ...
		 */
		if (EnumProcessModules(hCurrentProc, hAddtnlMods, sizeof(hAddtnlMods),
				&cbNeeded) != 0) {
			DWORD i;
...
void *dlsym(void *handle, const char *name) {
	...
	if (hModule == handle) {
		...
		for (pobject = &first_automatic_object; pobject; pobject =
				pobject->next) {
			if (pobject->hModule) {
				symbol = GetProcAddress(pobject->hModule, name);
				if (symbol != NULL)
